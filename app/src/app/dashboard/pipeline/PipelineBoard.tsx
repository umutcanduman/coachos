...
export default function PipelineBoard({ cardsByStage, totalsByStage }: Props) {
  ...
  async function onDrop(e: React.DragEvent, toStage: LifecycleStage) {
    e.preventDefault();
    setDragOver(null);
    ...
    const result = await moveClientStage(payload.cardId, toStage);
    if (!result.success) {
      setToast({ message: result.error ?? "Failed to move", type: "error" });
      return;
    }
    setToast({ message: `Moved to ${STAGE_LABELS[toStage]}`, type: "success" });
    startTransition(() => router.refresh());
  }

  async function handleQuickAction(card: PipelineCard) {
    // Stages with router-based actions instead of state changes:
    if (
    ...
    const result = await quickAdvance(card.id, card.lifecycle_stage);
    if (!result.success) {
                  ...
                  cards.map((card) => {
                    ...
                    return (
                      <div
                        ...
                        className="group rounded-lg border border-border bg-surface-2 p-3 transition-colors hover:border-border-2"
                      >
                        <Link
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {(stage === "lead" || stage === "discovery") && card.source && (
                              <div className="mt-1.5">
                                <span className="inline-flex rounded-full bg-c-blue-dim px-2 py-0.5 text-[0.65rem] font-medium text-c-blue">
                                  {card.source}
                                </span>
                              </div>
                            )}
